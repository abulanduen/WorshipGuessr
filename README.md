...
Data is shared, not local: the setlist, leaderboard, and audio clips all live in the cloud (Postgres + Vercel Blob), so everyone playing the deployed site sees the same setlist, regardless of who uploaded it.
...
1. Copy `.env.example` to `.env` and fill in:
   ...
   - `BLOB_READ_WRITE_TOKEN` — a [Vercel Blob](https://vercel.com/docs/storage/vercel-blob) read/write token. Audio clips are stored here, not on local disk — there's no local-storage fallback, so this is required even for local dev.
2. Install and run:
...
Under the hood, each file goes: original upload → analyzed and pushed to Blob as a "staging" file → (once you click Import) downloaded back, trimmed/encoded with ffmpeg, and the final short clip is pushed to Blob as the permanent one. The staging copy is deleted as soon as a row is imported, skipped as a duplicate, or removed from the review table.
...
2. In the project's **Storage** tab, create a Postgres database and a Blob store, and connect both to the project.
3. In **Settings → Environment Variables**, make sure there's a variable literally named `DATABASE_URL` pointing at your Postgres connection string (Vercel sometimes names it with a prefix like `postgres_DATABASE_URL` when connected through an integration — if so, add a plain `DATABASE_URL` entry with the same value, since Prisma is configured to read that exact name) and `BLOB_READ_WRITE_TOKEN` is present.
4. Redeploy. On first deploy, run `npx prisma migrate deploy` against the production `DATABASE_URL` (locally, with your `.env` pointed at production, or via a Vercel deploy hook) to create the schema.
...
- Vercel Blob for audio storage — trimmed clips and in-progress bulk-import staging files
